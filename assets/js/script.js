...
var populateList = function () {
	...
	for (i = 0; i < localSourceData.lists.length; i++) {
		...
		let newestPublishedDate = moment(
			localSourceData.lists[i].newest_published_date
		);
...
var populateBooks = function () {
	...
	name.textContent =
		localSourceData.lists[localSourceData.selected].display_name;
	...
	for (let i = 0; i < localSourceData.bookResults.books.length; i++) {
		...
		newBook.setAttribute(
			"class",
			"book bg-slate-100 p-1 m-2 rounded cursor-pointer"
		);
		newBook.setAttribute(
			"data-isbn13",
			localSourceData.bookResults.books[i].primary_isbn13
		);
		let bookTitle = document.createElement("h5");
		bookTitle.textContent = localSourceData.bookResults.books[i].title;
		bookTitle.setAttribute("class", "text-center h-16 font-semibold text-xl");
		let newRef = document.createElement("img");
		newRef.setAttribute("src", localSourceData.bookResults.books[i].book_image);
		newRef.setAttribute(
			"alt",
			localSourceData.bookResults.books[i].title + " book cover"
		);
		...
		bookAuthor.setAttribute("class", "text-sm text-center align-bottom");
...
var getBooks = function () {
	let Url =
		`https://api.nytimes.com/svc/books/v3/lists/${localSourceData.date}/` +
		`${
			localSourceData.lists[localSourceData.selected].list_name_encoded
		}.json?` +
		`api-key=${nytAPIKey}`;
		...
		.then(function (response) {
			if (response.ok) {
				response.json().then(function (data) {
					...
					if (localSourceData.selected > -1) getBooks();
				});
			} else {
				modalMessageEl.textContent = response.statusText;
				modalEl.style.display = "block";
